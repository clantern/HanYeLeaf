...
def test_new_item_initializes_part_weakness():
    ...
    item = system.add_item("item-a", ["part-a", "part-b"])

    system.review(item.id, True, now=1)

    assert item.introduced is True
    assert item.box == 1
    assert system.part_weakness["part-a"] == 0.9
    assert system.part_weakness["part-b"] == 0.9


def test_weak_part_enqueues_sibling_as_leaf():
    system = HanYeLeafSystem(weakness_threshold=1, leaf_delay=0)
    first = system.add_item("item-a", ["part-a", "part-b"])
    second = system.add_item("item-b", ["part-a", "part-c"])
