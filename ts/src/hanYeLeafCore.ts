...
const cloneState = (state: SystemState): SystemState =>
  JSON.parse(JSON.stringify(state)) as SystemState;

export function createSystem(overrides: Partial<SystemState> = {}, clock: Clock = () => 0, rng: RNG = Math.random): SystemCore {
...
export function review(core: SystemCore, itemId: number, success: boolean, options: { now?: number; kind?: ReviewKind } = {}): Item {
  ...
  for (const part of item.parts) {
    if (success) {
      state.part_weakness[part] = (state.part_weakness[part] ?? state.weakness_initial) * state.weakness_decay;
    } else {
      state.part_weakness[part] = (state.part_weakness[part] ?? state.weakness_initial) + state.weakness_increment;
    }
  }
...
export function traceEventSequence(core: SystemCore, events: TraceEvent[]): TraceStep[] {
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (event.action === "add_item") {
      const item = addItem(snapshotCore, event.question ?? "", event.parts ?? [], event.answer ?? "");
      step.item_id = item.id;
    } else if (event.action === "review") {
      const item = review(snapshotCore, event.item_id ?? 0, event.success ?? false, {
        now: event.now,
    ...
    const snapshot = snapshotState(snapshotCore.state);
